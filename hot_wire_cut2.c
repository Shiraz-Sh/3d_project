						...
						IrtHmgnMatType MatLocal)
{
    ...
    unsigned char *Grid;
    int *Px, *Py, *Ints, x, y, i, Guard, PMinY, PMaxY, NPts, PXVal, 
                                                 ...
                                                 int NumCtrl)
{
    ...
    IritPrsrVertexStruct *V, *Cur, *NV,
        *FirstV = NULL,
        *PrevV = NULL;
    IrtPtType *Pts, *TmpPts, WorldP;
    int i, It, Step, Im1, Ip1,
        NPts = 0,
        SmoothIters = 5;
					    ...
					    int *OutNumEdges)
{
    int i, j, k, StackTop, CurrentLoopCount, *EdgeCounts, *Visited, *Stack,
        MatchFwd, MatchRev, *CurrentLoopIndices, Curr, Match11, Match12, 
        Match21, Match22, Ei,
        MaxEdges = 0,
        NumEdges = 0,
        ...
        BestLoopCount = 0;
    IrtRType 
        CurrentLoopLength, CurrentLoopAvgZ, Len,
        BestLoopLength = -1.0,
...
*   2. Compute AABB and center the silhouette in the foam block. The input   *
*      model is NOT scaled; a warning is issued if it exceeds FoamWidth or   *
*      FoamHeight.                                                           *
*   3. Build an E3 B-spline curve: X=local_u_centered,                       *
*                                  Z=local_v_centered+FoamH/2               *
...
static IritPrsrObjectStruct *HWCBuildSilhouetteRuledSrf(
                                        ...
                                        const IritPrsrHWCDataStruct *Params)
{
    IritPrsrHWCEdgeStruct *BoundaryEdges;
    int k, n, Guard, b, *IsBoundary, Len, IDx, LeftIDx, RightIDx, Len1, Len2,
        BestStart = 0,
        BestLen = 0,
        NumBoundaryEdges = 0,
        TopStart = 0,
        TopLen = 0;
    IrtRType MinX, MinY, MaxX, MaxY, Cx, Cy, WxyLen,
             Diag, DistThresh, BottomThresh, MinXBottom, MaxXBottom, MaxYPath1,
             MaxYPath2, Sx, Sy, Wx, Wy, Wz, Slope, ZShift;
    IrtVecType uVec, vVec, wVec;
    IrtHmgnMatType MatLocal;
    IrtPtType *Pts2d, ProjP1, ProjP2, *TopPts;
    ...
    /* 5. Center the silhouette in the foam block (no scaling).              */
    /*    Warn the user if the model seems too large or too small.           */
    Cx = 0.5 * (MinX + MaxX);
    Cy = 0.5 * (MinY + MaxY);
    if ((MaxX - MinX) > Params -> FoamWidth ||
        (MaxY - MinY) > Params -> FoamHeight) {
        fprintf(stderr,
            "HWCBuildSilhouetteRuledSrf: WARNING - silhouette "
            "(%.1f x %.1f) exceeds foam block (%.1f x %.1f).\n",
            MaxX - MinX, MaxY - MinY,
            Params -> FoamWidth, Params -> FoamHeight);
    }
    else if ((MaxX - MinX) < Params -> FoamWidth * 0.1 &&
             (MaxY - MinY) < Params -> FoamHeight * 0.1) {
        fprintf(stderr,
            "HWCBuildSilhouetteRuledSrf: WARNING - silhouette "
            "(%.1f x %.1f) is very small relative to foam block "
            "(%.1f x %.1f). Model may be in wrong units.\n",
            MaxX - MinX, MaxY - MinY,
            Params -> FoamWidth, Params -> FoamHeight);
    }
    ...
    WxyLen = sqrt(wVec[0] * wVec[0] + wVec[1] * wVec[1]);
    ...
    if (BoundaryEdges != NULL && NumBoundaryEdges > 0) {
        ...
        for (k = 0; k < n; ++k) {
            ...
            for (b = 0; b < NumBoundaryEdges; ++b) {
                ...
                if (HWCDistPointSegment2D(Pts2d[k], ProjP1,
					  ProjP2) < DistThresh) {
                    IsBoundary[k] = 1;
                    break;
                }
        ...
        /* Find the longest contiguous sequence of IsBoundary == 0          */
	/* (wrapping around).						    */
        for (k = 0; k < n; ++k) {
            if (IsBoundary[k] == 0) {
                Len = 0;
    ...
    TopPts = (IrtPtType *) IritMalloc(sizeof(IrtPtType) * TopLen);

    for (k = 0; k < TopLen; ++k) {
        IRIT_PT_COPY(TopPts[k], Pts2d[(TopStart + k) % n]);
    }

    /* 8. Build silhouette curve mapped exactly into 3D World space.         */
    /*    The 2D contour is originally in the (uVec, vVec) plane of the      */
    /*     view. e rebuild the exact 3D orientation. 			     */
    /*									     */
    /*    The extruded surface must be aligned with wVec in 3D.		     */
    /*    The start of the extrusion will be -wVec * FoamDepth/2.	     */
    /*    We also shift the entire cut upwards to be centered in the foam    */
    /*    block.							     */
    ...
    for (k = 0; k < TopLen; ++k) {
        ...
        /* we must pre-extrapolate the Z-coordinate. The legacy code	     */
        /* computes aBuggy = Crv2_Z + slope * Crv2_Y_rot		     */
        /*          zBuggy = aBuggy - slope * MACHINE_MAX_Y		     */
	/* (where MACHINE_MAX_Y is 390.0).				     */
        /* But the correct math for extrapolating to Y = +/- 195.0 is:	     */
        /*   aTarget = Crv2_Z + slope * (195.0 - Crv2_Y_rot)		     */
        /* By setting aBuggy = aTarget, we can algebraically derive the      */
        /* exact Z shift needed per-point to cancel the bug out completely.  */
        /*   zShift = slope * (195.0 - 2 * Crv2_Y_rot)			     */
        /* After substituting Crv2_Y_rot =				     */
	/*			    -wz * slope + WxyLen * FoamDepth / 2,    */
	/* we get:							     */
        Slope = wVec[2] / WxyLen,
    ...
    /*    When IritPrsrHWCProcessRulingPair processes this surface, it      */
    /*    computes Angle = atan2(Dx, Dy) = atan2(wVec[0], wVec[1]), and     */
    /*    rotates the clamps accordingly to perfectly align the Y-axis of   */
    /*    the machine with wVec.					    */
                                 ...
                                 const IritPrsrHWCDataStruct *Params)
{
    ...
    for (Fi = 0; Fi < NumFiles; ++Fi) {
        ...
        while (fgets(Line, sizeof(Line), Fin) != NULL) {
            ...
            if (strncmp(Line,
			"; Performing vertical safe lift to safe Z height.",
			49) == 0) {
		    ...
		    /* angle for this view (each view resets                */
		    /* _TotalFoamRotation to 0).  Adding BOffset blindly    */
		    /* treated it as incremental, making the machine        */
		    /* accumulate extra full rotations.			    */
		    /* Instead, compute the shortest angular path.	    */
                    ...
                    /* Override any safe Z exits to provide massive         */
                    /* clearance for taller foam blocks.		    */
            ...
            /* Skip per-view Go Home commands; the combined file emits its  */
	    /* own G28 at the very end.					    */
    ...
    if (Params != NULL) {
        ...
        fprintf(Fout, "G1 X%.3f Y%.3f Z%.3f A%.3f B%.3f F300\n",
		x, y, z, a, LastB);
        /* Drop down to the slice height (1mm below the lowest point of the */
	/* object).							    */
        z = MinZSeen - 1.0;
    ...
    /* Emit G28 only to ensure firmware resets coordinate systems if        */
    /* necessary, but since we are already at 0,0,0,0,0 it won't physically */
    /* move sequentially.						    */
...
*                                                                            *
* KEYWORDS:                                                                  M
*   IritPrsrHWCCreatePath2, hot-wire, GCode, pipeline.		             M
*****************************************************************************/
IritPrsrObjectStruct *IritPrsrHWCCreatePath2(IritPrsrObjectStruct *Model,
					     const char *OutputGCodePath,
					     int NumViews,
					     int OutputITDType,
					     ...
					     IrtRType FineNess)
{
    ...
    IrtRType OldFineNess;
    char **GCodeFiles, GCodeFile[IRIT_LINE_LEN_LONG];
    int Vi, i,
        GCodeCount = 0;

    if (Model == NULL || OutputGCodePath == NULL || NumViews <= 0) {
        fprintf(stderr,
		"HWC: Invalid parameters.\n");
        return NULL;
    }
    ...
    {
        OldFineNess = IritPrsrFFCState.FineNess;
        if (FineNess > 0.0)
            IritPrsrFFCState.FineNess = FineNess;
        Solid = IritPrsrConvertFreeFormHierachy(Model, &IritPrsrFFCState,
						FALSE, FALSE);
...
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
    printf("HWC: Model ready. Type = %d.\n", Solid -> ObjType);
    fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */

    /* Save a copy as OBJ for visual inspection. */
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
    printf("HWC: Saving solid.obj...\n"); 
    fflush(stdout);
    if (IritPrsrOBJSaveFile(Solid, "solid.obj", FALSE, 2, 0))
        printf("HWC: Wrote solid.obj.\n");
    fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */

    /* 2. Select best view directions. */
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
    printf("SHWC: electing %d best view directions...\n", NumViews);
    fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
    ...
    if (GCodeFiles == NULL) {
        fprintf(stderr, "HWC: Failed to allocate GCode file list.\n");
	...
	HWCParams._SurfaceNum = Vi;

        /* 5a. Approximate silhouette contour. */
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
        printf("HWC: View %d: Computing silhouette contour...\n", Vi);
        fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
        ...
        if (Contour == NULL) {
            printf("HWC: View %d: contour approximation failed, skipping.\n",
		   Vi);
            continue;
        }
        ...
        if (!IritPrsrOBJSaveFile(Contour, ContourFile, FALSE, 0, 0))
            fprintf(stderr, "HWC: Failed saving %s.\n", ContourFile);
        else
            printf("HWC: Wrote %s\n", ContourFile);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
...
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
        printf("HWC: View %d: Building ruled surface...\n", Vi);
        fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
        ...
        if (RuledSrf == NULL) {
            printf("HWC: View %d: failed to build ruled surface, skipping.\n",
		   Vi);
            continue;
        }
...
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
        printf("HWC2: View %d: Generating GCode...\n", Vi);
        fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
        ...
        if (SimObj != NULL) {
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
            printf("HWC2: View %d: GCode written to %s\n", Vi, GCodeFile);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
            ...
            printf("HWC2: View %d: IritPrsrHWCCreatePath failed, skipping.\n",
		   Vi);
        }
    }
    ...
    if (GCodeCount > 0) {
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
        printf("HWC: Combining %d GCode files...\n", GCodeCount);
        fflush(stdout);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
        ...
        fprintf(stderr, "HWC2: No per-view GCode was produced.\n");
    }
...
#ifdef DEBUG_HOT_WIRE_CUT_ALG2
            printf("HWC2: All path geometries written to %s\n",
		   OutputITDFileName);
#endif /* DEBUG_HOT_WIRE_CUT_ALG2 */
        }
    }
